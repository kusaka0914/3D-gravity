...
Renderer3D::Renderer3D(Game* game) : Renderer(game)
{
    Initialize();
}
...
void Renderer3D::DrawScene(const glm::mat4& viewMat, const glm::mat4& projMat)
{
    ...
    if (!planets.empty()) {
        for (auto planet : planets) {
            glm::mat4 planetModel = glm::translate(glm::mat4(1.0f), planet->GetPos()) *
                                    glm::scale(glm::mat4(1.0f), glm::vec3(planet->GetScale()));
            glUniformMatrix4fv(locModel, 1, GL_FALSE, glm::value_ptr(planetModel));
            glUniform4f(locObjectColor, planet->GetColor().x, planet->GetColor().y, planet->GetColor().z,
                        planet->GetColor().w);
    ...
    if (players[0]->GetIsActive()) {
        DrawCharacter(players[0]->GetPos(), playerScale, glm::vec4(0.0f, 0.0f, 1.0f, 1.0f), players[0]->GetUpVec(),
                      players[0]->GetFacingYaw(), players[0]->GetMeshes());
    }

    bool isPlayer2Joined = mGame->GetIsPlayer2Joined();
    if (isPlayer2Joined) {
        DrawCharacter(players[1]->GetPos(), playerScale, glm::vec4(1.0f, 0.5f, 0.0f, 1.0f), players[1]->GetUpVec(),
                      players[1]->GetFacingYaw(), players[1]->GetMeshes());
    }
    ...
    if (!enemies.empty()) {
        for (auto enemy : enemies) {
            ...
            DrawCharacter(enemy->GetPos(), enemy->GetScale(), glm::vec4(0.0f, 1.0f, 0.0f, 1.0f), enemy->GetUpVec(),
                          enemy->GetFacingYaw(), enemy->GetMeshes());
    ...
    if (!NPCs.empty()) {
        for (auto NPC : NPCs) {
            if (NPC->GetIsActive()) {
                ...
                DrawCharacter(NPC->GetPos(), NPCScale, glm::vec4(0.4f, 0.25f, 0.1f, 1.0f), NPC->GetUpVec(),
                              NPC->GetFacingYaw(), NPC->GetMeshes());
            }
        }
    }
    ...
    if (!boats.empty()) {
        for (auto boat : boats) {
            if (boat->GetIsActive()) {
                ...
                DrawCharacter(boat->GetPos(), boatScale, glm::vec4(0.4f, 0.25f, 0.1f, 1.0f), boat->GetUpVec(),
                              boat->GetFacingYaw(), boat->GetMeshes());
            }
        }
    }
    ...
    if (!boatParts.empty()) {
        for (auto parts : boatParts) {
            if (parts->GetIsActive()) {
                ...
                DrawCharacter(parts->GetPos(), boatPartsScale, glm::vec4(0.4f, 0.25f, 0.1f, 1.0f), parts->GetUpVec(),
                              0.0f, parts->GetMeshes());
            }
        }
    }
    ...
    if (!crystals.empty()) {
        for (auto crystal : crystals) {
            if (crystal->GetIsActive()) {
                DrawCharacter(crystal->GetPos(), crystal->GetScale(), glm::vec4(0.4f, 0.25f, 0.1f, 1.0f),
                              crystal->GetUpVec(), 0.0f, crystal->GetMeshes());
            }
        }
    }
    ...
    if (!platforms.empty()) {
        for (auto platform : platforms) {
            if (platform->GetIsActive()) {
                DrawCharacter(platform->GetPos(), platform->GetScale(), glm::vec4(0.4f, 0.25f, 0.1f, 1.0f),
                              platform->GetUpVec(), 0.0f, platform->GetMeshes());
            }
        }
    }
};
...
std::vector<glm::mat4> Renderer3D::GetViews()
{
    ...
    if (isStageClear) {
        glm::mat4 view = mGame->GetCameraSystem()->GetPlayerView(mGame->GetPlayers()[0], 4.0f, true);
        views.emplace_back(view);
    }

    if (views.empty()) {
        glm::mat4 view = mGame->GetCameraSystem()->GetPlayerView(mGame->GetPlayers()[0], 10.0f);
        views.emplace_back(view);
    }

    bool isPlayer2Joined = mGame->GetIsPlayer2Joined();
    if (isPlayer2Joined) {
        glm::mat4 view = mGame->GetCameraSystem()->GetPlayerView(mGame->GetPlayers()[1], 10.0f);
        views.emplace_back(view);
    }
...
void Renderer3D::DrawAttackRange(Player* player)
{
    ...
    glm::vec3 forward = glm::normalize(glm::cross(worldLeft, upN) * std::cos(yaw) - worldLeft * std::sin(yaw));
