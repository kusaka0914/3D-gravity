...
Game::Game()
    : mWindow(nullptr), mSdlController(nullptr), mCurrentStage(nullptr),
      mCurrentStageNum(0), mHitStopTimer(-1.0f), mLastTime(0.0),
      mReloadKeyPressedPrev(false), mUIReloadKeyPressedPrev(false),
      mAPressedPrev(false), mIsPlayer2Joined(false),
...
void Game::UpdateGame() {
  ...
  const float deltaTime =
      std::min(0.04f, static_cast<float>(currentTime - mLastTime));
...
void Game::ChangeStage(int stageNum) {
  ...
  mCurrentStageYamlPath =
      "../assets/data/stage/stage" + std::to_string(stageNum) + ".yaml";
}
...
void Game::OnStarObtained() { mSceneSystem->OnStageClear(); }
...
void Game::OnStrongAttacked() { mSceneSystem->OnStrongAttacked(); }

void Game::OnLanded() { mSceneSystem->OnLanded(); }

void Game::OnPlayerDied() { mSceneSystem->OnPlayerDied(); }
...
void Game::FinishGame() { glfwSetWindowShouldClose(mWindow, GLFW_TRUE); }
...
void Game::StartPlayingScene() { mSceneSystem->StartPlayingScene(); }

void Game::StartFocusingScene() { mSceneSystem->StartFocusingScene(); }