...
bool Loader::loadPlayerFromYaml(class Game* game, const char* path, const std::vector<Planet>& planets,
...
bool Loader::loadEnemiesFromYaml(const char* path, const std::vector<Planet>& planets,
...
bool Loader::loadPlanetsFromYaml(Game* game, const char* path, std::vector<std::unique_ptr<Planet>>& outPlanets) {
    // // 惑星配列の初期化
    // outPlanets.clear();
    // try {
    //     YAML::Node root = YAML::LoadFile(path);
